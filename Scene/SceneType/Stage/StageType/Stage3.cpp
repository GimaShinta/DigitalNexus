...
void Stage3::StageLabel() const
{
    if (warning_label_state != WarningLabelState::None && warning_label_band_height > 1.0f)
    {
        ...
        // 紫色に変更（元: GetColor(120, 220, 255)）
        DrawLine(0, y_top, 1280, y_top, GetColor(180, 100, 255));
        DrawLine(0, y_bottom, 1280, y_bottom, GetColor(180, 100, 255));
        SetDrawBlendMode(DX_BLENDMODE_NOBLEND, 0);
        ...
        for (int i = 0; i < 8; ++i)
        {
            ...
            // 紫系に変更（元: GetColor(200, 255, 255)）
            DrawBox(glitch_x, glitch_y, glitch_x + glitch_len, glitch_y + 2, GetColor(200, 120, 255), TRUE);
            SetDrawBlendMode(DX_BLENDMODE_NOBLEND, 0);
        }
            ...
            )
        {
            ...
            DrawStringToHandle(stage_name_x, 320,
                "Data Abyss", GetColor(255, 255, 255), font_warning);  // 白でOK
